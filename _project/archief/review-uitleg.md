...
Uitleg vermeldt alleen `mm-`. Dat verklaart waarom `mřaɣa` (mř-), `mney` (m-), `nnedfes` (nn-), `nnuqzem` (nnu-) er allemaal anders uitzien — wat de leerling anders verwarrend vindt.
...
### 4.10 [DUBBELING] §4.4 partikels qa/tuɣa/aqqa/t̲ɣiř hoort in h8

In de uitleg-structuur staat §4.4 "De partikels qa en tuɣa — pseudo-werkwoorden", maar dat is precies wat hoofdstuk 8 (Pseudo-verbs) ook behandelt. Boek behandelt ze **alleen** in hoofdstuk 8 (p83–86) en in hoofdstuk 13 (p115–118 voor aspect-interactie).
...
| `t̲-t̲iwyeɣ` (in `necc waa t̲-t̲iwyeɣ ca`) | `awi` "nemen, trouwen" | structuur lijkt: `t̲-` 3SG:F-DO + `t̲iwyeɣ` 1SG-Imperfectief. Klopt patroon-wise, maar de dubbele `t̲t̲` aan het begin (clitic + prefix) verdient een gloss-uitleg |
...
- **`nnit̲`** "zelf, op eigen kracht" (klemtoon op werkwoord-onderwerp). Voorbeeld: `a t̲-awyeɣ nnit̲` "ik zal haar zelf trouwen".
- **`simant̲ n-`** "zelf" + persoonlijk voornaamwoord. Voorbeeld: `usiɣ-d necc simant̲ inu` "ik kwam zelf".
...
Uitleg meldt: *"`inu` 'van mij' (onregelmatig!)"* maar laat het daarbij. Boek p67 voetnoot zegt waarom: voor 1SG met `n` "van" is de regelmatige vorm `*ni`, maar in Iqeṛɛiyen Tarifit is dit altijd `inu`. Dat geldt **alleen** voor `n`. Bij andere voorzetsels gebruik je gewoon `-i`: `kiḏi` "met mij", `ɣari` "bij mij" (1SG-suffix). De uitleg zou minimaal moeten zeggen: *"De 1SG-vorm bij `n` is altijd `inu`, niet `ni`. Bij andere voorzetsels is 1SG gewoon `-i`."*
...
### 5.6 [SPELLING] §5.2.6 — `mmi-t̲ney` versus `mmi-tneɣ`
...
Voorstel: aanpassen naar `mmi-tneɣ`, `mmi-tsen` (gewone harde `t`).
...
Uitleg behandelt in §7.1 vier preverbale partikels: `ad`, `xad`, `ɣa`, `waa`. Maar **`qa`** wordt niet genoemd, terwijl het volgens boek p77 ook een preverbal element is (`qa` "present relevance"). Het wordt later wel behandeld in §4.4, §8.1, §13.4 — overal verspreid.
...
Boek p83 lijst **vijf** pseudo-werkwoorden in deze volgorde: `qa`, `tuɣa`, `aqqa`, `t̲ɣiř`, `ay`. Uitleg behandelt er **drie** (`aqqa`, `t̲ɣiř`, `ay`) en zegt: *"Voor `qa` en `t̲uya` — zie Hoofdstuk 4.4 hierboven."*
...
**Voorstel**: alle vijf pseudo-werkwoorden in h8 behandelen. In h4 alleen kort verwijzen (`qa` / `tuɣa` zijn pseudo-werkwoorden — zie h8 voor details).
...
Boek p85: *"In phrases where one would expect the preposition standing alone, the element `umi` is used."* Voorbeeld uit boek: `ḏyenni umi yenɣa ussen` "die mensen voor wie hij de jakhals had gedood".
...
- **post-verbal ontkenningspartikel**: `waa ssineɣ ca` "ik weet niet"
...
- `waa ssineɣ mani t̲eqqim` "ik weet niet waar ze is gebleven"
- `waa ssineɣ meřmi t̲awḏ` "ik weet niet wanneer ze aankomt"
...
2. **`ɣa` wordt gebruikt ipv `ad`** (zie h7 §7.3)
3. **Geen `i`** als relatieve marker (in tegenstelling tot gewone clefts)

Voorbeeld van wat een leerling moet weten: *"wat zal je doen?"* = `min ɣa t̲eggeḏ?`, niet `*min t̲a t̲eggeḏ?`. De uitleg behandelt dit gedeeltelijk in h15 §15.4 maar zou hier al moeten worden geïntroduceerd.
...
### 14.3 [SPELLING/ONDUIDELIJK] §14.3 — `uɣi-ɣa` "deze melk"

Uitleg geeft `d asemmam, uɣi-ɣa` "het is zuur, deze melk". CSV heeft `aɣi` "melk" — niet `uɣi`. De `uɣi`-vorm in de uitleg is de **Annexed State** van `aɣi` (`a` → `u` na voorzetsel of in post-topic-positie). Maar de uitleg legt dit niet uit — een leerling ziet `uɣi-ɣa` ineens en weet niet of dat een ander woord is.

**Voorstel**: gloss-noot toevoegen: *"`uɣi` is de AS-vorm van `aɣi` 'melk' — naamwoorden in post-topic-positie staan in AS (zie h3 §3.4)"*.
...
Voorbeeld uit boek: `missa i x ssaaseɣ řkas-nni` "de tafel waar ik dat glas op heb gezet". Hier staat `x` direct na `i` (relatieve marker), zonder pronominaal suffix. Dat is een bijzondere syntactische regel die in uitleg ontbreekt.
...
De kern *"Tarifit gebruikt twee volledige vervoegde werkwoorden naast elkaar"* is correct en sluit aan op boek p126 (let op: in het boek staat dit bij §16, niet p139 zoals uitleg meldt — kleine onnauwkeurigheid in pagina-referentie). Voorbeeld `xseɣ ad meřcey` met letterlijke gloss "ik wil ik zal trouwen" is helder.
...
- **Complementizer** (met `illa`/`belli`): `yessen illa ad ariɣ` "hij weet dat ik zal schrijven"
...
### 17.3 [FOUT] §17.2 voorbeeld `umi t̲-yenɣa, t̲ḥedd`

Uitleg: *"`umi t̲-yenɣa, t̲ḥedd` 'toen hij hem gedood had, stond ze op'"*. Boek p129 heeft de zin als `umi t̲-yenɣa, tbedd` — `tbedd` (3SG:F Perfectief van `bedd` "opstaan", CSV: `beḏḏ`).
...
### 17.4 [GROOT GAT] §17.2 — clitic-fronting/ɣa regels per voegwoord ontbreken

Boek p131–132 maakt expliciet voor elk voegwoord onderscheid:
- **Causes clitic-fronting + `ɣa`**: `umi`, `ṛami`, `xmi`, `xemmi`, `amen`, `qbeř`, `aṛ` (in temporeel gebruik)
- **Causes geen fronting, `ad` blijft `ad`**: `ḥama`, `ḥuma`, `puřki`, `lianna`, `daxataṛ`, `min-zi`, `henda`, `baš`

Dit is **leerbare** informatie — bepaalt of je `min ɣa t̲eggeḏ` of `min ad t̲eggeḏ` zegt na het voegwoord. Uitleg behandelt clitic-fronting in §7.3 in algemene termen, maar de **specifieke** regel per voegwoord komt nergens terug.

### 17.5 [ONDUIDELIJK] §17.2 voorbeeld `mři ḏ-usiɣ ifi cciɣ`
...
3. **Drie hoofdstukken hebben grote inhoudelijke gaten** die meer dan spelling vergen:
   ...
   - **h17** (Voegwoorden): per-voegwoord regels over clitic-fronting en `ɣa`/`ad`-keuze ontbreken
4. **Drie inconsistenties zitten verspreid over meerdere hoofdstukken**:
   - `dj` vs `ǧ`-notatie (h2, h13, h15, h18) — kies één
   - `qa`/`tuɣa` worden 3× behandeld (h4, h8, h13) — moet 1× in h8
