<!-- AUTO-GEGENEREERD uit nl/cursus.html door _project/scripts/gen_cursus_md.py
...
Tarifit heeft maar drie "echte" klinkers — `a`, `i` en `u` — plus de korte `e`. Elke klinker heeft twee uitspraken: een gewone, en een "donkere" variant naast een emfatische (donkere) medeklinker: `ḍ, ḏ̣, ṛ, ṣ, ṭ, ẓ`. Staat zo'n letter in een woord, dan worden álle klinkers in dat woord donker — de donkerte verspreidt zich.

| Letter | Gewoon | Donker (emfatisch) | Voorbeeld |
| --- | --- | --- | --- |
| `a` | als "a" in bad | als "a" in vader | `wa` "deze" · `iṭan` "honden" |
| `i` | als "i" in bit | nauwelijks verschil | `ini` "zeggen" · `ifri` "grot" |
| `u` | als "oe" in boek | als "ou" in jouw | `ru` "huilen" · `ṭuṛu` "zij baarde" |
...
- `netta ḏ uma` — hij is mijn broer
- `nettaṯ ḏ učma` — zij is mijn zus
- `ḏ uma-s` — het is zijn / haar broer
...
- `ṯɣiř-ayi ḏ ssehh` — "het lijkt mij waar / ik dacht dat het waar was"
- `ṯɣiř-asen ṯemmuṯ` — "ze dachten dat ze gestorven was"
- `waa ḏayi-ṯɣiř bu ḏ ssehh` — "ik dacht niet dat het waar was"
...
- `ḏ baba i d-yiwden` "het is mijn vader die is aangekomen"

#### Lijdend-voorwerp-cleft

- `(d) Mimun i ẓṛiɣ` "het is Mimoun die ik zag"
- `ḏ nečč i waa ṯ-yezrin` "het ben ik die hem niet zag"
...
| --- | --- |
| `ḏ netta i d-yusin` "hij is het die kwam" | `wi d-yusin?` "wie is gekomen?" |
| `ḏ Mimun i ẓṛiɣ` "Mimoun is wie ik zag" | `min d-yesya?` "wat heeft hij gekocht?" |
...
> `Am necc am wattas n yewdan mamec i ɛeqřey` "Zoals ik, zoals veel mensen, hoe ik mij herinner..."
